...
class SupCEResNet(nn.Module):
    ...
    def update_fc(self, nb_classes):
        ...
        # クラス数が増加しなければ何もしない
        if increment_size == 0 and self.fc is not None:
            return
    
        device = next(self.parameters()).device
        new_fc = self.generate_fc(self.feature_dim, nb_classes).to(device)

        # 既存 fc のパラメータをコピー
        if self.fc is not None and self.out_features > 0:
            old_fc: nn.Linear = self.fc
            new_fc.weight[: self.out_features].copy_(old_fc.weight)
            if old_fc.bias is not None and new_fc.bias is not None:
                new_fc.bias[: self.out_features].copy_(old_fc.bias)
